...
// Infrastructure 实现了infrastructure.Infrastructure以及
// 一个高层框架用Tarraform来实现infrastructure实现
//
// 这个实现是自动的:
//
//	* 通过目录服务Save/restore状态文件
//	* 在(w/ Terraform outputs)目录填充infrastructure数据
//	* 优雅的处理一些相关场景
//
type Infrastructure struct {
	// Creds 是一个收集凭证函数.参看helper/creds
	// 一个很好的helpder来实现这个函数
	CredsFunc func(*infrastructure.Context) (map[string]string, error)

	// VerifyCreds 是验证凭证的函数。参看helper/creds
	// 一个很好的helpder来实现这个函数
